# ===========================================================================
# Callaway & Sant'Anna (2021) Staggered DiD  ── R 実装 v7
# ===========================================================================
#
# v7 での修正点 (v6 からの変更):
...
output_dir    <- file.path(project_dir, "figures", "cs2021_did_r")
if (!dir.exists(output_dir)) dir.create(output_dir, recursive = TRUE)
log_file      <- file.path(output_dir, "run_progress.log")
panel_cache_file <- file.path(output_dir, "panel_cache_v7.rds")
...
#
#   実装方針:
#     never-treated (G=0) は両グループの対照群として共有する。
#     処理群のみをサブグループでフィルタする。
#     → CS(2021) の conditional PT はサブグループ内で成立を仮定。
# ---------------------------------------------------------------------------
run_subgroup <- function(panel_full, run_config) {
  ...
  for (off_val in c(1L, 0L)) {
    ...
    # never-treated + 対象サブグループ の処理群
    sub <- panel_full[(panel_full$G == 0L) | (panel_full$official == off_val), ]
