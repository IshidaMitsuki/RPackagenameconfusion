...
CONTROL_NEVER = "never_treated"
CONTROL_NOT_YET = "not_yet_treated"
CONTROL_MATCHED_NEVER = "matched_never_treated"


def selected_control_designs() -> list[tuple[str, str]]:
    all_specs = [
        ("A_never_treated", CONTROL_NEVER),
        ("B_not_yet_treated", CONTROL_NOT_YET),
        ("C_matched_never_treated", CONTROL_MATCHED_NEVER),
    ]
    raw = os.getenv("CONTROL_DESIGNS", "").strip()
    if not raw:
        return all_specs

    chosen = {x.strip() for x in raw.split(",") if x.strip()}
    specs = [item for item in all_specs if item[1] in chosen]
    if not specs:
        raise ValueError(
            "CONTROL_DESIGNS has no valid entries. Use one or more of: "
            "never_treated,not_yet_treated,matched_never_treated"
        )
    return specs
...
def _baseline_package_features(panel: pd.DataFrame) -> pd.DataFrame:
    """Build package-level baseline covariates for matched-control construction."""
    base = panel[panel["age_months"].between(0, 11)].copy()
    if base.empty:
        raise ValueError("No baseline observations found in age 0-11 months.")

    grp = base.groupby("Package", as_index=False)
    agg = grp.agg(
        n=("age_months", "size"),
        sum_x=("age_months", "sum"),
        sum_y=("y_log1p", "sum"),
        sum_xy=("y_log1p", lambda s: float((s * base.loc[s.index, "age_months"]).sum())),
        sum_x2=("age_months", lambda s: float((s * s).sum())),
        baseline_mean=("y_log1p", "mean"),
    )
    ...
    agg["baseline_slope"] = slope

    pkg_meta = (
        panel[["Package", "official_guidance", "is_treated", "is_control", "first_dl_month_id", "treat_month_id"]]
        .drop_duplicates(subset=["Package"])
        .copy()
    )

    out = pkg_meta.merge(
        agg[["Package", "baseline_mean", "baseline_slope"]],
        on="Package",
        how="left",
    )

    out["baseline_mean"] = out["baseline_mean"].fillna(out["baseline_mean"].median())
    out["baseline_slope"] = out["baseline_slope"].fillna(0.0)
    out["first_dl_month_id"] = out["first_dl_month_id"].fillna(out["first_dl_month_id"].median())
    return out


def build_matched_control_packages(
    panel: pd.DataFrame,
    official_filter: int | None = None,
    match_ratio: int = 2,
) -> pd.Index:
    """
    Build matched never-treated control package set.
    Matching uses baseline level/slope and first-download cohort month.
    """
    features = _baseline_package_features(panel)

    treated = features[(features["is_treated"] == 1) & features["treat_month_id"].notna()].copy()
    controls = features[features["is_control"] == 1].copy()

    if official_filter is not None:
        treated = treated[treated["official_guidance"] == official_filter].copy()
        controls = controls[controls["official_guidance"] == official_filter].copy()

    if treated.empty or controls.empty:
        return pd.Index([])

    cols = ["baseline_mean", "baseline_slope", "first_dl_month_id"]
    treated_x = treated[cols].to_numpy(dtype=float)
    control_x = controls[cols].to_numpy(dtype=float)

    # Standardize by pooled spread to avoid any single dimension dominating distances.
    pooled = np.vstack([treated_x, control_x])
    std = pooled.std(axis=0)
    std[std == 0] = 1.0
    treated_x = treated_x / std
    control_x = control_x / std

    k = max(1, min(match_ratio, len(controls)))
    picked: list[int] = []
    for i in range(treated_x.shape[0]):
        d = np.sum((control_x - treated_x[i]) ** 2, axis=1)
        nn = np.argpartition(d, k - 1)[:k]
        picked.extend(nn.tolist())

    matched_controls = controls.iloc[np.unique(picked)]["Package"]
    return pd.Index(matched_controls)
...
def build_event_sample(
    ...
    control_design: str = CONTROL_NEVER,
    matched_control_packages: pd.Index | None = None,
) -> pd.DataFrame:
    df = panel.copy()
    ...
    if control_design == CONTROL_NEVER:
        control = df["is_control"] == 1
    elif control_design == CONTROL_NOT_YET:
        control = (
            (df["is_treated"] == 1)
            & df["treat_month_id"].notna()
            & (df["month_id"] < shifted_treat_month_id)
        )
    elif control_design == CONTROL_MATCHED_NEVER:
        if matched_control_packages is None:
            raise ValueError("matched_control_packages is required for matched-never design.")
        control = (df["is_control"] == 1) & df["Package"].isin(matched_control_packages)
    else:
        raise ValueError(f"Unknown control design: {control_design}")
    ...
    control_in_sample = control & df["month_id"].isin(used_month_ids)
    control_in_sample = control_in_sample & ~treated_in_window

    sample = df[treated_in_window | control_in_sample].copy()
    sample["treated_in_sample"] = 0
    sample.loc[treated_in_window.loc[sample.index], "treated_in_sample"] = 1
    sample["control_in_sample"] = 0
    sample.loc[control_in_sample.loc[sample.index], "control_in_sample"] = 1
...
def run_event_study_model(
    ...
    n_control_pkg = int(sample.loc[sample["control_in_sample"] == 1, "Package"].nunique())
...
def run_event_study_suite(panel: pd.DataFrame) -> None:
    ...
    design_specs = selected_control_designs()

    for design_label, control_design in design_specs:
        print(f"  [Claim B] Control design: {design_label}")

        matched_controls = None
        if control_design == CONTROL_MATCHED_NEVER:
            matched_controls = build_matched_control_packages(panel, official_filter=None, match_ratio=2)
            print(f"    matched never-treated controls: {len(matched_controls):,} packages")

        # Main model (all official statuses pooled): asinh + log1p
        main_sample = build_event_sample(
            panel,
            window=MAIN_EVENT_WINDOW,
            control_design=control_design,
            matched_control_packages=matched_controls,
        )
        results.append(
            run_event_study_model(
                ...
                outcome="y_asinh",
                window=MAIN_EVENT_WINDOW,
                label=f"{design_label}: Main pooled",
                output_prefix=f"claim_b_{control_design}_main",
                control_design=control_design,
            )
        )
        results.append(
        ...
        # Window robustness on asinh outcome
        for w in [12, 36]:
            sample_w = build_event_sample(
                panel,
                window=w,
                control_design=control_design,
                matched_control_packages=matched_controls,
            ...
            results.append(
                run_event_study_model(
                    ...
                    outcome="y_asinh",
                    window=w,
                    label=f"{design_label}: Window robustness {w}",
                    output_prefix=f"claim_b_{control_design}_window",
                    control_design=control_design,
                )
            )
        ...
        for off in [1, 0]:
            matched_controls_g = matched_controls
            if control_design == CONTROL_MATCHED_NEVER:
                matched_controls_g = build_matched_control_packages(
                    panel,
                    official_filter=off,
                    match_ratio=2,
                )
            sample_g = build_event_sample(
                panel,
                window=MAIN_EVENT_WINDOW,
                official_filter=off,
                control_design=control_design,
                matched_control_packages=matched_controls_g,
            )
            results.append(
                run_event_study_model(
                    sample=sample_g,
                    outcome="y_asinh",
                    window=MAIN_EVENT_WINDOW,
                    label=f"{design_label}: Official subgroup {label_official(off)}",
                    output_prefix=(
                        f"claim_b_{control_design}_subgroup_"
                        f"{'official_yes' if off == 1 else 'official_no'}"
                    ),
                    control_design=control_design,
                )
            )
        ...
        results.append(
            run_event_study_model(
                ...
                outcome="y_asinh",
                window=MAIN_EVENT_WINDOW,
                label=f"{design_label}: Placebo (shift -12 months)",
                output_prefix=f"claim_b_{control_design}_placebo",
                control_design=control_design,
            )
        )
...
def run_att_interaction(panel: pd.DataFrame) -> None:
    ...
    for design_label, control_design in design_specs:
        ...
        sample["treated_post_x_official"] = (
            sample["treated_post"] * sample["official_guidance"].astype(float)
        )
